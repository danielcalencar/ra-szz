...
//	    String rawTypeName = stripQualifiedTypeName(stripTypeParamsFromTypeName(type.toString()));
	...
	public static int startPositionFirstStatement(MethodDeclaration decl) {
		return new StatementCounter().startPositionFirstStatement(decl);
	}
	public static int startPositionFirstStatement(AbstractTypeDeclaration decl) {
		return new StatementUtil().startPositionFirstStatement(decl);
	}
	
	private static class StatementCounter extends ASTVisitor {
		private int counter;
		private int startPositionFirstStatement;
		public int countStatements(MethodDeclaration methodDeclaration) {
			counter = 0;
			startPositionFirstStatement = 0;
			methodDeclaration.accept(this);
			return counter;
		}
		public int startPositionFirstStatement(MethodDeclaration declaration) {
			counter = 0;
			startPositionFirstStatement = 0;
			declaration.accept(this);
			return startPositionFirstStatement;
		}
		@Override
		public void preVisit(ASTNode node) {
			if (node instanceof Statement && !(node instanceof Block)) {				
				counter++;
				if (counter == 1) startPositionFirstStatement = node.getStartPosition();
			}
		}
	}
	private static class StatementUtil extends ASTVisitor {
		private int counter;
		private int startPositionFirstStatement;
		public int startPositionFirstStatement(AbstractTypeDeclaration declaration) {
			counter = 0;
			startPositionFirstStatement = 0;
			declaration.accept(this);
			return startPositionFirstStatement;
		...
		@Override
		public void preVisit(ASTNode node) {
			if (node instanceof SimpleName && counter++ == 0) 
				startPositionFirstStatement = node.getStartPosition();			
		}
	}
	/*
	private static class StatementUtil extends ASTVisitor {
		...
		public int startPositionFirstStatement(MethodDeclaration methodDeclaration) {
			counter = 0;
			startPositionFirstStatement = 0;
			methodDeclaration.accept(this);
			return startPositionFirstStatement;
	...
	private static class StatementUtil extends ASTVisitor {
		...
		private int i;
		public String firstStatement(MethodDeclaration methodDeclaration) {
			i = 0;
		...
		@Override
		public void preVisit(ASTNode node) {
			if (!(node instanceof MethodDeclaration) && i++ == 0) 
				startPositionFirstStatement = node.getStartPosition();
			
			if (node instanceof Statement || node instanceof Block) {				
				stop = true;
				startPositionFirstStatement = node.getStartPosition() - 1;
			} else if (!stop) {
