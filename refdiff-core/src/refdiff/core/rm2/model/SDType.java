...
public class SDType extends SDContainerEntity {
	...
	public SDType(SDModel.Snapshot snapshot, int id, String simpleName, SDContainerEntity container,
			String sourceFilePath) {
		this(snapshot, id, simpleName, container.fullName() + "." + simpleName,
				new EntityKey(container.key() + "." + simpleName), container, sourceFilePath, false, false);
	}

	private SDType(SDModel.Snapshot snapshot, int id, String anonymousId, SDType container, String sourceFilePath,
			boolean localType) {
		this(snapshot, id, "" + anonymousId, container.fullName() + "$" + anonymousId,
				new EntityKey(container.key() + "$" + anonymousId), container, sourceFilePath, true, localType);
	}

	private SDType(SDModel.Snapshot snapshot, int id, String simpleName, String fullName, EntityKey key,
			SDContainerEntity container, String sourceFilePath, boolean isAnonymous, boolean localType) {
	...
	public void addReference(SDEntity entity) {
		long callerline = entity.getCallerline();
		entity.addReferencedBy(this, callerline);
	}

	@Override
	public void addReferencedBy(SDType type, long line) {
		this.referencedBy.add(type, line);
	}
	...
	public String getVerboseFullLine() {
		...
		if (isInterface())
			sb.append("interface " + simpleName());
		else
			sb.append("class " + simpleName());
		/*if (subtypes().size() > 0)
			sb.append(" extends ");
		for (SDType subtype : subtypes()) {
			sb.append(subtype.simpleName + ",");
		}
		// String result =
