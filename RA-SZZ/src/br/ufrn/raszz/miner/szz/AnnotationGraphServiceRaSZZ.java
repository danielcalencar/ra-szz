...
public class AnnotationGraphServiceRaSZZ extends AnnotationGraphService {
				...
				for(Line linetotrace : hunk.getContent()){
					...
					if(linetotrace.getType() == LineType.DELETION){
						...
						if(!isCommentOrBlankLine(linetotrace.getContent()) && !isImport(linetotrace.getContent())){   
							...
							//verifica se contém refatoração no fixing
							boolean isrefac = szzDAO.hasRefacFix(beforeRev.getPath(), fixRev.getRevision(), linetotrace.getPreviousNumber(), linetotrace.getAdjustmentIndex(), linetotrace.getContent());
							if (isrefac) {
								...
								createLinesInPreviousRevisions(model,linetotrace, beforeRev, fileRevisions, fixRev, false, 0, 0, false, null, 0, startlinetotrace);//, true, true);
							}
						}
					}
				}
	...
	private void createLinesInPreviousRevisions(AnnotationGraphModel model, Line linetotrace, SzzFileRevision rev,
					...
					//, boolean israszz, boolean ismaszz) 
					throws Exception{
		...
		if(prevrev != null) {
			...
			for(Line line : previousLines){				
				...
				/*if (prevrev.getRevision().equals("57533")) //log.info(prevcontent);
				{*/
				...
				//#DEBUG
				//if (prevcontent.equals("}staticCollection<IdentityHashSet<Tuple>>getEquivalenceClasses(LOFilterop,Map<LogicalOperator,DataBag>derivedData){}")) {
				//	log.info("line to trace: " + content + "#" + linetotrace.getPreviousNumber() + "- r:" + linetotrace.getRevision() + "- p:" + linetotrace.getPreviousRevision());
				//	log.info("prevline     : " + prevcontent + "#p" + line.getPreviousNumber() + "-#n"+ line.getNumber() + "- r:" + line.getRevision() + "- p:" + line.getPreviousRevision());
				//}/*
			...
			//IF mudou de local o arquivo?	#RE-TRACE	
			String prevrefpath = RefacOperationsUtil.prevRefacPath(refacSet, linetotrace.getPreviousPath(), linetotrace.getPreviousRevision(), linetotrace.getPreviousNumber(), linetotrace.getAdjustmentIndex(), content);
			if (prevrefpath != null) {
				retrace(model, linetotrace, fileRevisions, fixRev, isrefac, indexPosRefac, indexChangePath, rev, prevrefpath, indexFurtherBack, startline);
			} else {
	...
	private void retrace(AnnotationGraphModel model, Line linetotrace, LinkedList<SzzFileRevision> fileRevisions, SzzFileRevision fixRev, boolean isrefac, int indexPosRefac,
		...
		else fileRevisions = null; //TODO Ver essa implementação
